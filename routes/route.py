...
from models.user import User, UserLogin, LoginRequest, SecurityQuestionRequest, ResetPasswordRequest
from utils.auth import verify_password, hash_password, create_access_token, ACCESS_TOKEN_EXPIRE_MINUTES
load_dotenv()
router = APIRouter()
...
async def reset_password(request: ResetPasswordRequest):
    ...
    if user.get("answer", "").lower() != request.answer.lower():
        raise HTTPException(status_code=400, detail="Incorrect security question answer")
