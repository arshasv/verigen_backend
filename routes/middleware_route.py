...
# URL configurations
VERILOG_PROCESS_URL = os.getenv(
    "VERILOG_PROCESS_URL", "http://0.0.0.0:8000/process-verilog/"
)
ADDITIONAL_API_URL = os.getenv(
    "ADDITIONAL_API_URL", "http://localhost:5000/run_openlane/"
)
    ...
    async def start_consuming(self, timeout: int = 30):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.connection.process_data_events(time_limit=1)
            )
...
async def process_verilog_file(file_id: str):
    try:
        ...
        if not notification_result:
            raise HTTPException(
                status_code=500, detail="Timeout waiting for RabbitMQ notification"
            )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if not isinstance(api_response, dict):
                raise HTTPException(
                    status_code=500, detail="Invalid response from additional API"
                )
